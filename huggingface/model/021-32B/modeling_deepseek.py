...
def load_if(condition, layer_idx, name):
    if not condition:
        return None
    return torch.load(mg_name_creator(layer_idx, name))
...
class DeepseekV2MLP(nn.Module):
    def __init__(self, config, layer_idx, hidden_size=None, intermediate_size=None):
        super().__init__()
        self.config = config
    ...
    def __init__(self, config, layer_idx):
        ...
        if config.n_shared_experts is not None:
            ...
            self.shared_experts = DeepseekV2MLP(
                config=config, intermediate_size=intermediate_size, layer_idx=layer_idx
            )
