...
def patch_after_import_torch_pu1():
    # 1. Patch for torch.xxx
    torch.cuda.is_available = torch.pu1.is_available
    torch.cuda.current_device = lambda : f'privateuseone:{torch.pu1.current_device()}'
    torch.cuda.device_count = torch.pu1.device_count
    torch.cuda.set_device = torch.pu1.set_device
    torch.cuda.DoubleTensor = torch.pu1.DoubleTensor
    torch.cuda.FloatTensor = torch.pu1.FloatTensor
    torch.cuda.LongTensor = torch.pu1.LongTensor
    torch.cuda.HalfTensor = torch.pu1.HalfTensor
    torch.cuda.BFloat16Tensor = torch.pu1.BFloat16Tensor
    torch.cuda.IntTensor = torch.pu1.IntTensor
    torch.cuda.synchronize = torch.pu1.synchronize
    torch.cuda.get_rng_state = torch.pu1.get_rng_state
    torch.cuda.set_rng_state = torch.pu1.set_rng_state
    torch.cuda.random.get_rng_state = torch.pu1.get_rng_state
    torch.cuda.synchronize = torch.pu1.synchronize
    torch.cuda.empty_cache = torch.pu1.empty_cache
    torch.Tensor.cuda = torch.Tensor.pu1
    torch.cuda.manual_seed = torch.pu1.manual_seed
    torch.cuda.Event = torch.pu1.Event
    torch.cuda.Stream = torch.pu1.Stream
    torch.cuda.stream = torch.pu1.stream
    torch.cuda.get_device_properties = torch.pu1.get_device_properties
    # add torch.musa.current_devce() to activate torch.musa.default_generators
    d = torch.pu1.current_device()
    torch.cuda.default_generators = torch.pu1.default_generators
    # torch.cuda.amp = torch.musa.amp
    # Memory
    torch.cuda.memory_allocated = torch.pu1.memory_allocated
    torch.cuda.max_memory_allocated = torch.pu1.max_memory_allocated
    torch.cuda.memory_reserved = torch.pu1.memory_reserved
    torch.cuda.max_memory_reserved = torch.pu1.max_memory_reserved
    torch.cuda.memory._record_memory_history = torch.pu1.memory._record_memory_history
    torch.cuda.memory._snapshot = torch.pu1.memory._snapshot

    # (yehua.zhang) replace lazy_call to avoid cpu memory leak,
    # because failure of cuda init in lazy_call will cause endless operation of emplace back.
    torch.cuda._lazy_call = torch.pu1.lazy_init._lazy_call
    torch.cuda._lazy_init = torch.pu1.lazy_init._lazy_init
    ...
    def musa_type(*args, **kwargs):
        result = orig_type(*args, **kwargs)
        if isinstance(result, str):
            result = result.replace("privateuseone", "cuda")
        return result
    torch.Tensor.type = musa_type
