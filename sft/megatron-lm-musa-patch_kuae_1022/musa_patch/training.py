...
def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        if args.log_throughput:
            ...
            log_string += f' tokens_per_gpu_per_second: {tokens_per_gpu_per_second:.2f} |'
            if args.log_timers_to_tensorboard:
                if writer:
                    writer.add_scalar('throughput', throughput, iteration)
                    writer.add_scalar('MFU', mfu, iteration)
                    writer.add_scalar('TGS', tokens_per_gpu_per_second, iteration)
                if wandb_writer:
                    wandb_writer.log({'throughput': throughput}, iteration)
                    wandb_writer.log({'MFU': mfu}, iteration)
                    wandb_writer.log({'TGS': tokens_per_gpu_per_second}, iteration)
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    with maybe_enable_profiling(
        ...
        while iteration < args.train_iters:
            ...
            try:
                should_save_by_bs_changed = _GLOBAL_NUM_MICROBATCHES_CALCULATOR.current_global_batch_size % _GLOBAL_NUM_MICROBATCHES_CALCULATOR.micro_batch_times_data_parallel_size != 0
            except:
                should_save_by_bs_changed = False
            if (get_num_microbatches() != num_microbatches or should_save_by_bs_changed) and iteration != 0:
