...
def run_monitor(args):
    ...
    last_update_time = time.time()  # 新增：最后更新时间戳
    MAX_NO_UPDATE_TIME = 1800       # 30分钟无更新阈值
    training_rank=False
    ...
    with open(args.log_path, 'r') as f:
        ...
        while True:
            ...
            # 新增：30分钟无更新检测
            if current_time - last_update_time > MAX_NO_UPDATE_TIME and training_rank:
                time_record.error = time.time()
                error_msg = f"日志文件 {args.log_path} 超过30分钟未更新，可能训练已中断"
                
                # 记录事件
                save_events(args, f'日志无更新超时 \n[POD_ID] {args.pod_id}', time_stamp=time_record.error)
                
                # 发送通知
                content_ding = f'[{args.model_name}] 监控异常: ' + error_msg
                content_ding += f'\n{common_info}'
                content_log = content_ding + f'\n[最后更新时间] {time_record.timestamp2date(last_update_time)}'
            ...
            if line_num == 1:
                ...
                print_content(args, None, str(time_record.train_start))
            ...
            elif key_words.training and 'elapsed time per iteration (ms):' in line:
                training_rank=True
            # elif "throughput (token/sec/GPU) :" in line:
                ...
                try:
                    ...
                    ding_error = f'钉钉消息发送失败!!! {e} \n 失败消息内容: elapsed time per iteration (ms) lines:' + line 
                    print("====================================elapsed time per iteration (ms) lines:===line===================================="+line)
                    output_log(ding_error, log_type='ERROR')
                iter_time_cur = float(re.findall(r'\d+\.\d+', line.split('elapsed time per iteration (ms):')[1])[0])/1000 # s 当前iter耗时
