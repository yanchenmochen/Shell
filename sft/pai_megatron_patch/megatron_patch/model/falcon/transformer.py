...
class ParallelAttention(MegatronModule):
    ...
    def forward(self, hidden_states, position_ids, attention_mask,
        ...
        if self.use_flash_attn:
            query_layer = query_layer.reshape(batch_size, self.num_attention_heads_per_partition, q_length,
                                              self.head_dim).transpose(0, 2).transpose(1, 2)
            key_layer = key_layer.reshape(batch_size, self.num_kv, q_length, self.head_dim).transpose(0, 2).transpose(1,
                                                                                                                      2)
