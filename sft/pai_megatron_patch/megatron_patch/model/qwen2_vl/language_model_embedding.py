        ...
        if self.config.sequence_parallel:
            if not self.reduce_scatter_embeddings:
                ...
                if image_embeds is not None:
                    ...
                    embeddings = embeddings.masked_scatter(image_input_mask,
                                                           image_embeds.to(embeddings.device, embeddings.dtype))
                if video_embeds is not None:
                    video_input_mask = video_input_mask.transpose(0, 1).unsqueeze(-1).expand_as(embeddings)
                    embeddings = embeddings.masked_scatter(video_input_mask,
                                                           video_embeds.to(embeddings.device, embeddings.dtype))
                embeddings = embeddings.transpose(0, 1).contiguous()
