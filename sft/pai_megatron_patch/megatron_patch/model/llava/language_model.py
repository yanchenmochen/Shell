    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
        ...
        for batch_idx, cur_input_ids in enumerate(enc_input_ids):
            ...
            while image_token_indices.numel() > 0:
                ...
                if getattr(self.args, 'tune_mm_mlp_adapter', False) and getattr(self.args, 'mm_use_im_start_end',
                                                                                False):
                    cur_new_input_embeds.append(cur_input_embeds[cur_start:image_token_start - 1].unsqueeze(1).detach())
                    cur_new_input_embeds.append(cur_input_embeds[image_token_start - 1:image_token_start].unsqueeze(
                        1))  # special token: <im_start>
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_input_embeds.append(cur_input_embeds[image_token_start + 1:image_token_start + 2].unsqueeze(
                        1))  # special token: <im_end>
