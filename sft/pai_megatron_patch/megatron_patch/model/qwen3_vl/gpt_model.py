...
class GPTModel(LanguageModule):
    ...
    def __init__(
        ...
        # ! Besides, VP is also not supported in the current impl.
        if self.pre_process and len(self.config.deepstack_visual_indexes) > self.decoder.num_layers_per_pipeline_rank:
            raise NotImplementedError(
                "DeepStack requires extra comm logic when the num_layers of first pipeline stage "
                "are smaller than len(deepstack_visual_indexes)."
            )
