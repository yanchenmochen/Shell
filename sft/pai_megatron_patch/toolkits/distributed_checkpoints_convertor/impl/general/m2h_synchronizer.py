...
class MG2HFSynchronizer(BaseSynchronizer):
    ...
    def __init__(self, load_dir, model_provider_func=None):
        ...
        self._rank_mapping[self.rank] = torch.Tensor(
            [self.tp_rank, self.pp_rank, self.etp_rank, self.ep_rank, self.dp_rank, self.edp_rank]).to(self.device)
    ...
    def set_selfattn_state(self, attn, hf_attn):
        ...
        num_query_groups = (
            self.args.num_query_groups if self.args.group_query_attention else self.args.num_attention_heads)
    ...
    def check_and_save(self, output_dir):
        ...
        for bucket_idx in range(max_n_local_buckets):
            ...
            if self.debug:
                logging.info(
                    f"[Iters {bucket_idx} RANK {self.rank}] starts synchronizing parameters with other ranks...")
            if len(ops) > 0:
                ...
                if self.debug:
                    for op in ops:
                        if op.op == dist.isend:
                            logging.info(
                                f"[Iters {bucket_idx} RANK {self.rank}] ({self.rank} -> {op.peer}) with {op.tensor.numel() * op.tensor.dtype.itemsize / 2 ** 20} MiB.")
                        else:
                            logging.info(
                                f"[Iters {bucket_idx} RANK {self.rank}] ({op.peer} -> {self.rank}) with {op.tensor.numel() * op.tensor.dtype.itemsize / 2 ** 20} MiB.")
                for req in reqs:
                    req.wait()
            if self.debug:
            ...
            for param_id, data_dict in data.items():
                ...
                if data[param_id].shape != global_shape[param_id]:
                    raise ValueError(
                        f"Unexpected shape on {key}. Expected: {global_shape[param_id]}, but {data[param_id].shape}")
    ...
    def _collect_dist_info(self):
        ...
        # merge_type
        global_merge_type = torch.zeros([self.world_size, self.hf_size], dtype=self._merge_type.dtype,
                                        device=self.device)
        ...
        for remote_rank_id, remote_merge_type in enumerate(global_merge_type):
            if self.debug:
                ...
                if (self._merge_type[and_mask] != remote_merge_type[and_mask]).any():
                    ...
                    raise ValueError(
                        f"Find mismatched merge_type between local rank {self.rank} and remote rank {remote_rank_id} on key {key}")
    ...
    def _build_p2p_ops(self, required_keys: List[str]):
        ...
        if self.debug:
            ...
            send_recv_pattern = torch.zeros([self.hf_size, self.world_size, self.world_size], dtype=torch.int,
                                            device=self.device)
        ...
        for param_id, has_data in enumerate(self._has_param[:, self.rank]):
            ...
            for remote_rank, should_send in enumerate(required_ids[:, param_id]):
                ...
                if data.device != self.device:
                    logging.warning(
                        f"Find unexpected device {data.device} on key {self._id_to_hf_params_key[param_id]}, moving to {self.device}")
            ...
            if send_recv_pattern.sum() != 0:
                for param_id, pattern_per_param in enumerate(send_recv_pattern):
                    if pattern_per_param.sum() != 0:
                        raise ValueError(
                            f"Mismatched send/recv ops detected on key {self._id_to_hf_params_key[param_id]}: {pattern_per_param}.")
