...
from configuration_RW import RWConfig, RWConfig_40b
...
# The simple map of names for "automated" rules.
megatron_to_transformers_opmap = {
    "input_layernorm": "ln_attn",
    ...
    "self_attention.key_value": "self_attention.query_key_value",
}
...
def get_megatron_sharded_states(args, tp_size, pp_size, pp_rank):
    ...
    for i in range(tp_size):
        ...
        optim_name = [i for i in os.listdir(os.path.join(args.load_path, sub_dir_name)) if 'optim' in i]
        for name in optim_name:
            print(
                f'Deleting Optimizer states ({os.path.join(args.load_path, sub_dir_name)}/{name}) for faster convertion.')
            os.system(f'rm -rf {os.path.join(args.load_path, sub_dir_name)}/{name}.pt')
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    if len(sub_dirs) == 1:
        ...
        if args.model_name == "falcon-7b" or args.model_name == "falcon-40b":
            num_checkpoints = len(sub_dirs) - 1
            state_dict = merge_transformers_sharded_states_7b(args.load_path, num_checkpoints)
        else:
            raise ValueError("Please set correct model name")

    config = RWConfig.from_pretrained(args.load_path)
    internal_state_dict = {}
    for layer_id in range(config.num_hidden_layers):

        internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.query.weight'] = \
            state_dict['transformer.h.' + str(layer_id) + '.self_attention.query_key_value.weight'][:config.hidden_size,
            :]

        internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.key_value.weight'] = \
            state_dict['transformer.h.' + str(layer_id) + '.self_attention.query_key_value.weight'][config.hidden_size:,
            :]

        qkv_dtype = state_dict['transformer.h.' + str(layer_id) + '.self_attention.query_key_value.weight'].dtype
        internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.key_value.bias'] = \
            torch.zeros(state_dict['transformer.h.' + str(layer_id) + '.self_attention.query_key_value.weight'][
                        config.hidden_size:, :].shape[0],
                        dtype=qkv_dtype)

        internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.dense.weight'] = \
            state_dict['transformer.h.' + str(layer_id) + '.self_attention.dense.weight']

        internal_state_dict['transformer.layers.' + str(layer_id) + '.mlp.dense_h_to_4h.weight'] = state_dict[
            'transformer.h.' + str(layer_id) + '.mlp.dense_h_to_4h.weight']

        internal_state_dict['transformer.layers.' + str(layer_id) + '.mlp.dense_4h_to_h.weight'] = state_dict[
            'transformer.h.' + str(layer_id) + '.mlp.dense_4h_to_h.weight']

        if args.model_name == 'falcon-7b':
            internal_state_dict['transformer.layers.' + str(layer_id) + '.input_layernorm.weight'] = state_dict[
                'transformer.h.' + str(layer_id) + '.input_layernorm.weight']

            internal_state_dict['transformer.layers.' + str(layer_id) + '.input_layernorm.bias'] = state_dict[
                'transformer.h.' + str(layer_id) + '.input_layernorm.bias']
        else:
            internal_state_dict['transformer.layers.' + str(layer_id) + '.input_layernorm.weight'] = state_dict[
                'transformer.h.' + str(layer_id) + '.ln_attn.weight']

            internal_state_dict['transformer.layers.' + str(layer_id) + '.input_layernorm.bias'] = state_dict[
                'transformer.h.' + str(layer_id) + '.ln_attn.bias']

            internal_state_dict['transformer.layers.' + str(layer_id) + '.post_attention_layernorm.weight'] = \
            state_dict[
                'transformer.h.' + str(layer_id) + '.ln_mlp.weight']

            internal_state_dict['transformer.layers.' + str(layer_id) + '.post_attention_layernorm.bias'] = state_dict[
                'transformer.h.' + str(layer_id) + '.ln_mlp.bias']

    internal_state_dict["transformer.word_embeddings.weight"] = state_dict['transformer.word_embeddings.weight']
    internal_state_dict["transformer.final_layernorm.weight"] = state_dict['transformer.ln_f.weight']
    internal_state_dict["transformer.final_layernorm.bias"] = state_dict['transformer.ln_f.bias']
    ...
    for i in range(args.target_tensor_model_parallel_size):
        ...
        lm_head_dict["weight"] = out_lm_head[i].clone()
        ...
        for layer in range(num_layers):
            ...
            for layer_name in layers_to_copy:
                ...
                # handle attention K, V, Q weights
                    ...
                    layer_name = f"layers.{layer}.self_attention.key_value.{weight_or_bias}"

                # handle attention K, V, Q bias
                elif op_name.startswith("self_attn.key_value") and weight_or_bias == "bias":
