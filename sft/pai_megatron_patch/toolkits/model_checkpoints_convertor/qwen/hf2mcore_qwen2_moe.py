...
def load_megatron_model(args):
    ...
    if (
        ...
        if args.target_decoder_first_pipeline_num_layers is not None:
            ...
            pp_layers_per_stage = [
                                      args.num_layers // args.pipeline_model_parallel_size] * args.pipeline_model_parallel_size
                ...
                for pp_rank in range(args.pipeline_model_parallel_size):
                    ...
                    for k, v in split_state.items():
                        ...
                        try:
                            ...
                            tgt = re.sub(r"decoder.layers.\d+",
                                         "decoder.layers." + str(layers_to_copy[(pp_rank, int(res[0]))]), k)
    ...
    with torch.no_grad():
        ...
        for mglayer, hflayer in zip(mgmodel.decoder.layers, hfmodel.model.layers):
            ...
            if args.add_qkv_bias:
                ...
                q_bias, k_bias, v_bias = torch.split(qkv_bias,
                                                     split_size_or_sections=[q_dim_per_group, kv_dim_per_group,
                                                                             kv_dim_per_group], dim=1)
...
def convert_checkpoint_from_transformers_to_megatron(hfmodel: Qwen2MoeForCausalLM, mgmodel: GPTModel, args):
    ...
    with torch.no_grad():
        ...
        for layer_idx, (mglayer, hflayer) in tqdm(enumerate(zip(mgmodel.decoder.layers, hfmodel.model.layers)),
                                                  total=len(mgmodel.decoder.layers)):
            ...
            qkv_proj = torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=1).view(-1,
                                                                                            args.hidden_size).contiguous()
