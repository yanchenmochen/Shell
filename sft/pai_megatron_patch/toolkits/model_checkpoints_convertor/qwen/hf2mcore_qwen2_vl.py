    ...
    if (
        ...
        for tp_rank in range(args.tensor_model_parallel_size):
            for pp_rank in range(args.pipeline_model_parallel_size):
                checkpoint_name = get_checkpoint_name(model_path, iteration, release, True, tp_rank, pp_rank, None,
                                                      None)
                print(f'load {checkpoint_name}')
                keys = ['model']
...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hfmodel, args):
    ...
    n_params = sum(
        [t.numel() for k, t in mgvision.state_dict().items() if isinstance(t, torch.Tensor) and 'extra_state' not in k])
    ...
    for mglayer, hflayer in zip(mgllm.decoder.layers, hfllm.layers):
        ...
        q_bias, k_bias, v_bias = torch.split(qkv_bias, split_size_or_sections=[q_dim_per_group, kv_dim_per_group,
    ...
    n_params = sum(
        [t.numel() for k, t in hfllm.state_dict().items() if isinstance(t, torch.Tensor) and 'extra_state' not in k])
    assert n_params == copied_numel
    ...
    if (
        ...
        for tp_rank in range(args.tensor_model_parallel_size):
            for pp_rank in range(args.pipeline_model_parallel_size):
                    ...
                    for k, v in full_model.items():
                        if check_layer(layers_to_copy, k):
                            ...
                            k = re.sub(r"decoder.layers.\d+", f"decoder.layers.{layers_to_copy[int(res[0])]}", k)
                                ...
                                "word_embeddings" in k or "output_layer" in k or "final_layernorm" in k or 'vision_model' in k):
                            continue
                        if 'vision_model' in k:
                ...
                save_state_dict(args, model_chunk, checkpoint_name,
