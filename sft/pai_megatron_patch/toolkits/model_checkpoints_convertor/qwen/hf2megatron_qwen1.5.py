    ...
    for layer_id in range(config.num_hidden_layers):
        ...
        if args.model_name != "qwen1.5-32b":
            ...
            internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.key_value.weight'] = torch.cat(
                (k_weight, v_weight))
            internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.key_value.bias'] = torch.cat(
                (k_bias, v_bias))
        ...
        for layer in range(num_layers):
            ...
            for i in range(args.target_tensor_model_parallel_size):
                ...
                if args.model_name == "qwen1.5-32b":
                    ...
                    group_query_weight = query_weight.view(num_groups // args.target_tensor_model_parallel_size,
                                                           num_heads // num_groups * head_dim, 1)
                    group_kv_weight = kv_weight.view(num_groups // args.target_tensor_model_parallel_size, 2 * head_dim,
                                                     1)
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    intermediate_size_map = {1024: 2816, 2048: 5504, 2560: 6912, 4096: 11008,
                             5120: 13824 if args.model_name != 'qwen1.5-32b' else 27392, 6656: 17920, 7168: 19200,
                             8192: 22016}
    config = Qwen2Config(
        bos_token_id=151643,
        eos_token_id=151645,
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Transpose the Q matrix for query for Llama70b.
                ...
                if weight_or_bias == 'weight':
                    tp_states = [
                        i.view(num_groups // args.target_tensor_model_parallel_size, query_dim + kv_dim, hidden_size)
                        for i in tp_states]
                    ...
                    tp_states = [i.view(num_groups // args.target_tensor_model_parallel_size, query_dim + kv_dim) for i
                                 in tp_states]
    ...
    print(
        f"\n\nNOTE: Conversion is completed! Make sure to copy *.json files from Huggingface dir to {args.save_path}"
