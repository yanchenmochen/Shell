    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Transpose the QKV matrix.
                ...
                all_qkvs = [i.reshape(num_groups // args.target_tensor_model_parallel_size, -1) for i in
                            torch.chunk(params, args.target_tensor_model_parallel_size, 0)]
                split_size = heads // num_groups * hidden_size_per_head
                all_qs = torch.cat([i[:, :split_size].reshape(-1) for i in all_qkvs])
                all_kvs = torch.cat([i[:, split_size:].reshape(-1) for i in all_qkvs])
                ...
                output_state_dict[layer_name + f".self_attn.q_proj.bias"] = out_q.clone()
                output_state_dict[layer_name + f".self_attn.k_proj.bias"] = out_kv[0].clone()
                output_state_dict[layer_name + f".self_attn.v_proj.bias"] = out_kv[1].clone()
