...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    for layer_id in range(config.num_hidden_layers):
        internal_state_dict['transformer.layers.' + str(layer_id) + '.self_attn.query_key_value.weight'] = \
            state_dict['model.layers.' + str(layer_id) + '.self_attn.W_pack.weight']
        ...
        internal_state_dict['transformer.layers.' + str(layer_id) + '.input_layernorm.bias'] = \
            torch.zeros(internal_state_dict['transformer.layers.0.input_layernorm.weight'].shape,
                        dtype=input_layernorm_dtype)

        internal_state_dict['transformer.layers.' + str(layer_id) + '.post_attention_layernorm.weight'] = \
            state_dict['model.layers.' + str(layer_id) + '.post_attention_layernorm.weight']

        internal_state_dict['transformer.layers.' + str(layer_id) + '.post_attention_layernorm.bias'] = \
            torch.zeros(internal_state_dict['transformer.layers.0.post_attention_layernorm.weight'].shape,
                        dtype=input_layernorm_dtype)
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Transpose the QKV matrix.
                ...
                output_state_dict[layer_name + f".self_attn.W_pack.weight"] = out_val.clone()
