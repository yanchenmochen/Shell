...
megatron_to_transformers_opmap = {
    ...
    "mlp.dense_h_to_4h": "mlp.c_fc",
    "mlp.dense_4h_to_h": "mlp.c_proj",
    "self_attention.query": "attn.c_attn",
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    config = GPTBigCodeConfig(
        ...
        use_cache=True,
        bos_token_id=getattr(megatron_args, "bos_token_id", 0),
        eos_token_id=getattr(megatron_args, "eos_token_id", 0),
        pad_token_id=getattr(megatron_args, "pad_token_id", 0),
    ...
    position_embeddings = get_element_from_dict_by_path(
        tp_state_dicts[0], "model.language_model.embedding"
    )

    if 'position_embeddings.weight' in position_embeddings:
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            if 'key_value' not in op_name:
                output_state_dict[key.replace('layers', 'transformer.h').replace(op_name, hf_op_name)] = params
            else:
                original = output_state_dict[key.replace('layers', 'transformer.h').replace(op_name, hf_op_name)]
                output_state_dict[key.replace('layers', 'transformer.h').replace(op_name, hf_op_name)] = torch.cat(
                    [original, params])
