    ...
    for k, v in mid_state.items():
        try:
            ...
            if not isinstance(v[0], torch.Tensor):
                target_v = v[0]
            elif contains(k, ['router', 'gate', 'input_layernorm', 'pre_mlp_layernorm', 'enorm', 'hnorm',
                              'layer_norm_weight', 'final_layernorm']):
                target_v = v[0]
            elif contains(k, ['word_embeddings', 'output_layer', 'linear_q_down_proj', 'linear_q_up_proj',
                              'linear_kv_down_proj', 'linear_kv_up_proj.layer_norm_weight', 'eh_proj',
                              'linear_q_proj']):
                target_v = torch.cat(v, dim=0)
            elif 'linear_kv_up_proj' in k:
                viewed = [x.view(group_per_split, -1, q_head_dim - args.qk_pos_emb_head_dim + args.v_head_dim,
                                 args.kv_lora_rank) for x in v]
...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hfmodel, args):
    ...
    if args.mtp_num_layers and args.mtp_num_layers > 0:
        ...
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.q_a_proj.weight"] = mtplayer.self_attention.linear_q_down_proj.weight
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.q_b_proj.weight"] = mtplayer.self_attention.linear_q_up_proj.weight
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.q_a_layernorm.weight"] = mtplayer.self_attention.linear_q_up_proj.layer_norm_weight
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.kv_a_proj_with_mqa.weight"] = mtplayer.self_attention.linear_kv_down_proj.weight
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.kv_b_proj.weight"] = mtplayer.self_attention.linear_kv_up_proj.weight
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.self_attn.kv_a_layernorm.weight"] = mtplayer.self_attention.linear_kv_up_proj.layer_norm_weight
        ...
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.mlp.gate.e_score_correction_bias"] = mtplayer.mlp.router.expert_bias.float()
        ...
        for i in range(args.num_experts):
            ...
            mtp_dict[f"model.layers.{mtp_layer_idx}.mlp.experts." + str(i) + ".down_proj.weight"] = getattr(
                mtplayer.mlp.experts.linear_fc2, 'weight' + str(i))
        ...
        mtp_dict[
            f"model.layers.{mtp_layer_idx}.mlp.shared_experts.down_proj.weight"] = mtplayer.mlp.shared_experts.linear_fc2.weight
        # output norm
    ...
    for layer_idx, (mglayer, hflayer) in enumerate(zip(mgmodel.decoder.layers, hfmodel.model.layers)):
        ...
        if not hasattr(mglayer.mlp, 'router'):
            ...
            gate_weight, up_weight = torch.split(mglayer.mlp.linear_fc1.weight,
...
def convert_checkpoint_from_transformers_to_megatron(hfmodel, mgmodel, args):
    ...
    if args.mtp_num_layers and args.mtp_num_layers > 0:
        ...
        mtplayer.pre_mlp_layernorm.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.post_attention_layernorm.weight"])
        mtplayer.self_attention.linear_q_down_proj.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.q_a_proj.weight"])
        mtplayer.self_attention.linear_q_up_proj.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.q_b_proj.weight"])
        mtplayer.self_attention.linear_q_up_proj.layer_norm_weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.q_a_layernorm.weight"])
        mtplayer.self_attention.linear_kv_down_proj.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.kv_a_proj_with_mqa.weight"])
        mtplayer.self_attention.linear_kv_up_proj.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.kv_b_proj.weight"])
        mtplayer.self_attention.linear_kv_up_proj.layer_norm_weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.kv_a_layernorm.weight"])
        mtplayer.self_attention.linear_proj.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.self_attn.o_proj.weight"])
        mtplayer.mlp.router.weight.copy_(mtp_dict[f"model.layers.{mtp_layer_idx}.mlp.gate.weight"])
        mtplayer.mlp.router.expert_bias.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.mlp.gate.e_score_correction_bias"])
        if args.moe_grouped_gemm == True:
            for i in range(args.num_experts):
                ...
                linear_fc2_weighti.copy_(
                    mtp_dict[f"model.layers.{mtp_layer_idx}.mlp.experts." + str(i) + ".down_proj.weight"])
        else:
            for i in range(args.num_experts):
        ...
        mtplayer.mlp.shared_experts.linear_fc2.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.mlp.shared_experts.down_proj.weight"])
        # output norm
        mgmodel.mtp.layers[0].final_layernorm.weight.copy_(
            mtp_dict[f"model.layers.{mtp_layer_idx}.shared_head.norm.weight"])
    ...
    for (tp_rank, etp_rank, ep_rank, pp_rank) in generate_rank_group(
        ...
        for k, v in full_model.items():
            ...
            if check_layer(layers_to_copy, k):
                ...
                k = re.sub(r"decoder.layers.\d+", "decoder.layers." + str(layers_to_copy["decoder.layers." + res[0]]),
                           k)
            elif 'mtp' in k:
                if not has_mtp:
                    continue
            ...
            if not isinstance(v, torch.Tensor):
                target_v = v
            elif contains(k, ['linear_q_down_proj', 'linear_kv_down_proj', 'linear_q_proj', 'linear_q_up_proj',
                              'linear_kv_up_proj', 'linear_q_proj']) and 'norm' not in k:
