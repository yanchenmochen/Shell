# QwQ 模型在Pai-Megatron-Patch的最佳实践
...
mkdir qwen-ckpts
cd qwen-ckpts
git clone https://modelscope.cn/models/Qwen/QwQ-32B
...
32B \
/mnt/qwen-ckpts/QwQ-32B \
/mnt/qwen-ckpts/QwQ-32B-hf-to-mcore-te-tp4-pp1  \
4  \
1  \
bf16 \
true \
...
在QwQ中，我们已将预训练和微调整合到`run_mcore_qwen.sh`脚本，对于不同的使用场景，二者各参数的意义有所不同。
...
备注：当`AC=offload`或`full`时，可设置`MP_AC_LAYERS`
环境变量来控制Checkpointing或Offload的TransformerLayer层数（默认值：`1`）。

```bash
cd /workspace/Pai-Megatron-Patch/examples/qwq
...
10000  \
100   \
/workspace/output_mcore_qwq_pretrain
```
...
10000  \
100   \
/workspace/output_mcore_qwq_finetune
```
...
0.5B \
/mnt/qwen-ckpts/QwQ-32B-hf-to-mcore-te-tp4-pp1  \
/mnt/qwen-ckpts/QwQ-32B-mcore-te-to-hf    \
1  \
1  \
bf16 \
true \
...
--model hf \
--model_args pretrained=/mnt/qwen-ckpts/QwQ-32B-mcore-te-to-hf,trust_remote_code=True \
