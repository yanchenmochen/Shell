...
def forward_backward_no_pipelining(
    *,
    ...
    if not forward_only:
        model.module.module.is_last_batch = True
        backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
        model.module.module.is_last_batch = False
    ...
    *,
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            #HACK(sherry):打印每个weight的grad norm, last stage
            if last_iteration and parallel_state.is_pipeline_last_stage():
                model.module.module.is_last_batch = True
            #HACK(sherry)
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            #HACK(sherry):打印每个weight的grad norm, first and middle stage
            if i == num_warmup_microbatches - 1:
                model.module.module.is_last_batch = True
            #HACK(sherry)
            input_tensor_grad = backward_step(
    ...
    #HACK(sherry):打印每个weight的grad norm
    model.module.module.is_last_batch = False
    #HACK(sherry)
    return forward_data_store
