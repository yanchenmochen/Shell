...
def _custom_rope(
    ...
    if False and self.config.apply_rope_fusion:
        ...
        k_no_pe, value = torch.split(
            kv, [config.qk_head_dim, config.v_head_dim], dim=-1)
        ...
        if k_pos_emb.ndim == 4:
            k_pos_emb = k_pos_emb.expand(-1, -1, num_attention_heads_per_partition, -1)
        else:
            assert k_pos_emb.ndim == 3
            k_pos_emb = k_pos_emb.expand(-1, num_attention_heads_per_partition, -1) 
        key = torch.cat([k_no_pe, k_pos_emb], dim=-1)

        q_split_start = config.qk_head_dim
        q_split_end = q_split_start + config.qk_pos_emb_head_dim
...
class CustomPreAttention(nn.Module):
    """
    封装 Megatron 的 _custom_forward_before_attention 调用。
    主要用于在 attention 前处理 query/key/value 等输入。
    """
    def __init__(self, config=None):
        super().__init__()
        self.config = config  # 可选：如果你想传配置对象，可以在 forward 使用
    ...
    def forward(
        ...
        """
        执行自定义的 pre-attention 前向逻辑。

        参数：
            q_compressed: 压缩后的 query 张量
            kv_combined: KV 合并后的张量
            attention_mask: 注意力掩码
            key_value_states: 历史 KV 状态（或 cache）
            position_ids: 位置 id
            packed_seq_params: 可选参数（支持 packed sequence）
            inference_context: 推理上下文（Megatron 特有）
        """
        query, key, value = _custom_rope(
            self.config,
            q,
            kv,
            k_pos_emb,
...
class MLASelfAttention(MultiLatentAttention):
    ...
    def __init__(
        ...
        self.pre_attn = CustomPreAttention(config=self.config)
