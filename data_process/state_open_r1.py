...
if __name__ == "__main__":
    r1_file=" /mnt/seed-program-nas/001688/honghong/zjlab-megatron/zj_examples/sft/OpenR1-Math-220k/openr1_full.jsonl"
    r1_file="openr1_full.jsonl"
    # r1_file= "r1_2000.jsonl"
    r1_pkl = "openr1_full.pkl"
    tokenizer = AutoTokenizer.from_pretrained("/mnt/self-define/songquanheng/deepseek_v2_lite_with_special_tokens_auto/")
    if not os.path.exists(r1_pkl):
        sample_data = []
        with open(r1_file, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                data = json.loads(line)
                sample_data.append(data)
        with open(r1_pkl, "wb") as f:
            print(f"数据已保存到 {r1_pkl}")
            pickle.dump(sample_data, f, protocol=pickle.HIGHEST_PROTOCOL)

    else:
        with open(r1_pkl, "rb") as f:
            print(f"读取序列化文件{r1_pkl}")
            start = time.time()
