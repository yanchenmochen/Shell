...
def get_multi_turn_distribution(sample_data):
    role_seqs = []
    for sample_idx, sample in enumerate(sample_data[:5000]):
        conversations = sample.get('conversations', [])
        role_seq = []
        for messages in conversations:
            role_seq.append(messages['from'])
...
def get_distribute(sample_data):
    ...
    for sample_idx, sample in enumerate(sample_data[:5000]):
        ...
        for messages in conversations:
            message_alpaca = {}
            # 当前仅仅支持gpt， human
            message_alpaca['role'] = "user" if messages['from'] == 'human' else 'assistant'
            message_alpaca['content'] = messages['value']
            sample.append(message_alpaca)
        all_ids = tokenizer.apply_chat_template(sample)
        sample_lens.append(len(all_ids))

        # 每个包含1轮对话位于0号位， 19轮对话保存超过>=20轮以上的内容
        chat_turn = len(conversations) // 2
        if  chat_turn < 20:
...
if __name__ == "__main__":
    kod_file="/mnt/seed-program-nas/001688/honghong/zjlab-megatron/zj_examples/sft/KodCode-V1-SFT-R1/kodcode_full.jsonl"
    kod_file="kodcode_full.jsonl"
    kod_pkl = "kod.pkl"
    tokenizer = AutoTokenizer.from_pretrained("/mnt/self-define/songquanheng/deepseek_v2_lite_with_special_tokens_auto/")
    if not os.path.exists(kod_pkl):
        sample_data = []
        with open(kod_file, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                data = json.loads(line)
                print(f"数据已保存到 {kod_pkl}")
                sample_data.append(data)
        with open(kod_pkl, "wb") as f:
            pickle.dump(sample_data, f, protocol=pickle.HIGHEST_PROTOCOL)

    else:
        with open(kod_pkl, "rb") as f:
            print(f"读取序列化文件{kod_pkl}")
